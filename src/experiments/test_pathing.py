...
def mat_sqr_diff(A, B):
    A_ = np.transpose(A)
    return np.sum(np.square(A_), axis=1, keepdims=True) - 2 * np.matmul(A_, B) + np.sum(np.square(B), axis=0, keepdims=True)
...
if __name__ == '__main__':
    ...
    # distances = mat_sqr_diff(np.transpose(all_reps), next_rep.detach().cpu().numpy())
    distances = mat_sqr_diff(np.transpose(encoded_rep.detach().cpu().numpy()), encoded_next_rep.detach().cpu().numpy())
    print(distances)
