...
@partial(jax.jit, static_argnames=['next_state_dims', 'memory_size', 'batch_size'])
def compute_value_score(Q, params, next_state_dims, memory_size, batch_size):
    ...
    v = jnp.reshape(v, (batch_size, next_state_dims))
    return v, s
...
def compute_error(Q, V, S, M, params, r_key, next_state_dims, memory_size, batch_size, value_access=True):
    ...
    V_ = jnp.reshape(V_, (batch_size, next_state_dims))
...
def make_query(s, t, context_length, input_dims, target_dims):
    ...
    query = jnp.reshape(batch, (-1, input_dims * context_length + target_dims))
    return query
...
class Model(base.Model):
    ...
    def __init__(self, dims, context_length, hidden_size, memory_size=16, value_access=True, lr=0.01, iteration=0, r_seed=42):
        ...
        r_key, subkey = jax.random.split(r_key)
        key = jax.random.normal(subkey, (hidden_size, self.input_dims * context_length + self.target_dims)) * 0.1
        
        r_key, subkey = jax.random.split(r_key)
        value_0 = jax.random.normal(subkey, [hidden_size, self.memory_size * self.next_state_dims]) * 0.1
    ...
    def infer(self, s, t, context=None):
        ...
        best_value, best_score = compute_value_score(query, self.params, self.next_state_dims, self.memory_size, batch_size)
