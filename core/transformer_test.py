...
if __name__ == "__main__":
    ...
    dims = 1024
    model = transformer.Model(dims, 8, 128, [128, 64], 8, r_seed=millis)

    r_key = jax.random.key(millis)
    r_key, subkey = jax.random.split(r_key)
    s = jax.random.normal(subkey, (1, 1, dims))
    r_key, subkey = jax.random.split(r_key)
    t = jax.random.normal(subkey, (16, dims))

    r_key, subkey = jax.random.split(r_key)
    S = jnp.concatenate([jnp.tile(s, (16, 1, 1)), jax.random.normal(subkey, (16, 31, dims), dtype=jnp.float32)], axis=1)
    X = jnp.roll(S, -1, axis=1)
    T = jnp.tile(jnp.expand_dims(t, axis=1), (1, 32, 1))
    r_key, subkey = jax.random.split(r_key)
    scores = jax.random.uniform(subkey, (16, 32), minval=0.9, maxval=1.0)
    ...
    print("Value:", value_[0, :10], "Expected:", X[0, 0, :10])
    print("Diff:", jnp.mean(jnp.abs(value_[0, :] - X[0, 0, :])))